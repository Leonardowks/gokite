...
-- Atualizar a função calcular_detalhes_transacao para usar as categorias corretas da tabela tax_rules
...
BEGIN
  -- Mapear origem da transação para categoria fiscal (agora usando nomes da tabela tax_rules)
  CASE NEW.origem
    WHEN 'aula' THEN categoria_fiscal := 'aula';
    WHEN 'pacote' THEN categoria_fiscal := 'pacote';
    WHEN 'aluguel' THEN categoria_fiscal := 'aluguel';
    WHEN 'venda_produto' THEN categoria_fiscal := 'venda_produto';
    WHEN 'ecommerce' THEN categoria_fiscal := 'ecommerce';
    WHEN 'trade_in' THEN categoria_fiscal := 'trade_in';
    WHEN 'pousada' THEN categoria_fiscal := 'pousada';
    WHEN 'hospedagem' THEN categoria_fiscal := 'pousada';
    ELSE categoria_fiscal := 'aula'; -- Default para serviços
  END CASE;
